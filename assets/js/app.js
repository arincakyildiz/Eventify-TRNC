...
function renderAdminUserList() {
  ...
  container.innerHTML = "";
  const sorted = [...managedUsers].sort((a, b) =>
  ...
  sorted.forEach((user) => {
    ...
    item.innerHTML = `
      <div class="ef-list-item-main">
        <span class="ef-list-item-title">${user.name}</span>
        <span class="ef-list-item-meta">${user.email} · ${user.city}</span>
      </div>
      <div class="ef-list-item-actions">
        <span class="ef-chip">
          <span class="ef-chip-dot"></span>
        ...
        <button class="ef-btn ef-btn-ghost" data-edit-user="${user.id}">Edit</button>
        <button class="ef-btn ef-btn-danger" data-delete-user="${user.id}">Delete</button>
      </div>
    `;

    item
      .querySelector("[data-edit-user]")
      .addEventListener("click", () => fillUserForm(user));
    item
      .querySelector("[data-delete-user]")
      .addEventListener("click", () => deleteManagedUser(user.id));
...
function fillUserForm(user) {
  document.getElementById("managed-user-id").value = user.id;
  document.getElementById("managed-user-name").value = user.name;
  document.getElementById("managed-user-email").value = user.email;
  document.getElementById("managed-user-city").value = user.city;
  document.getElementById("managed-user-role").value = user.role ?? "citizen";
}
...
function setupManagedUsersForm() {
  ...
  form.addEventListener("submit", (e) => {
    ...
    const payload = {
      ...
      city: document.getElementById("managed-user-city").value.trim(),
      role: document.getElementById("managed-user-role").value || "citizen",
    };
...
function setupRegistrationForm() {
  ...
  if (backBtn) {
    backBtn.addEventListener("click", () => closeRegistrationForm());
  }
